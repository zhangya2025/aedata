...
(function($) {
	...
	$('#name').on('keyup',function(e){
		...
		if ( typeof the7_type_data != 'undefined' ) {
			if (the7_type_data.existing_post_types.hasOwnProperty(value) && value !== original_slug) {
				$slugexists.removeClass('hidemessage');
			} else {
				$slugexists.addClass('hidemessage');
			}
		}
		if ( typeof the7_tax_data != 'undefined' ) {
			if (the7_tax_data.existing_taxonomies.hasOwnProperty(value) && value !== original_slug) {
				$slugexists.removeClass('hidemessage');
			} else {
				$slugexists.addClass('hidemessage');
			}
	...
	$('.the7-taxonomy-submit').on('click',function(e){
		if ( $('.the7-table :checkbox:checked').length == 0 ) {
			e.preventDefault();
