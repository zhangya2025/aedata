...
class AEGIS_System {
    ...
    public function register_admin_menu() {
        ...
        if (AEGIS_System_Roles::user_can_manage_warehouse() && (!empty($states['sku']) || !empty($states['dealer_master']) || !empty($states['codes']) || !empty($states['shipments']) || !empty($states['public_query']))
        ) {
            $has_visible = true;
        }
        if (AEGIS_System_Roles::user_can_use_warehouse() && !AEGIS_System_Roles::user_can_manage_warehouse() && !empty($states['shipments'])) {
            $has_visible = true;
        }
...
class AEGIS_System_Schema {
    ...
    protected static function get_table_sql($charset_collate) {
        ...
        return [$audit_sql, $media_sql, $sku_sql, $dealer_sql, $code_batch_sql, $code_sql, $shipment_sql, $shipment_item_sql, $query_log_sql];
    }
...
class AEGIS_Shipments {
    ...
    public static function render_admin_page() {
        ...
        if ('POST' === $_SERVER['REQUEST_METHOD']) {
            $idempotency = isset($_POST['_aegis_idempotency']) ? sanitize_text_field(wp_unslash($_POST['_aegis_idempotency'])) : null;
            $whitelist = ['shipment_action', 'dealer_id', 'shipment_no', 'codes', '_wp_http_referer', 'aegis_shipments_nonce', '_aegis_idempotency'];
            $validation = AEGIS_Access_Audit::validate_write_request(
                $_POST,
                [
        ...
        echo '<p class="aegis-t-a6">订单关联暂未开放，出库单仅绑定经销商与扫码记录。</p>';
    ...
    protected static function handle_create_request($data) {
        ...
        $inserted = $wpdb->insert(
            ...
            [
                ...
                'order_ref'   => null,
                'status'      => 'created',
                'meta'        => wp_json_encode(['count' => count($validated_codes)]),
            ],
            ['%s', '%d', '%d', '%s', '%s', '%s', '%s']
        );
    ...
    protected static function render_create_form() {
        ...
        echo '<p class="aegis-t-a6">订单关联暂未启用，如需关联请等待订单模块上线。</p>';
...
class AEGIS_Public_Query {
    ...
    public static function render_admin_page() {
        ...
        if ('POST' === $_SERVER['REQUEST_METHOD']) {
            $action = isset($_POST['public_query_action']) ? sanitize_key(wp_unslash($_POST['public_query_action'])) : '';
            $whitelist = ['public_query_action', 'code_value', 'aegis_public_query_nonce', '_wp_http_referer', 'reset_code'];
            $validation = AEGIS_Access_Audit::validate_write_request(
                $_POST,
                [
                    'capability'   => AEGIS_System::CAP_MANAGE_WAREHOUSE,
            ...
            if (!$validation['success']) {
                ...
                if ('reset_b' === $action) {
                    $code_value = isset($_POST['reset_code']) ? sanitize_text_field(wp_unslash($_POST['reset_code'])) : '';
                    $reset = self::reset_b_counter($code_value);
                    if (is_wp_error($reset)) {
                        $errors[] = $reset->get_error_message();
                    } else {
                        $messages[] = 'B 计数已清零。';
                    }
                } else {
        ...
        if ($result) {
            ...
            wp_nonce_field('aegis_public_query_action', 'aegis_public_query_nonce');
            echo '<input type="hidden" name="public_query_action" value="reset_b" />';
            echo '<input type="hidden" name="reset_code" value="' . esc_attr($result['code']) . '" />';
            echo '<p class="submit"><button type="submit" class="button">清零 B 计数</button></p>';
            echo '</form>';
        }
     ...
     * 重置 B 计数。
     */
    protected static function reset_b_counter($code_value) {
        global $wpdb;
        $code_value = trim((string) $code_value);
        if ('' === $code_value) {
            return new WP_Error('empty_code', '请输入要清零的防伪码。');
        }
        ...
        $wpdb->query($wpdb->prepare("UPDATE {$table} SET query_b_offset = query_b_count WHERE id = %d", $record->id));
