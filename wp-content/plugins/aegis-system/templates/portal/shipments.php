    ...
    <?php if (!$shipment) : ?>
        ...
        <div class="aegis-shipments-tabs" data-aegis-tabs>
            <div class="aegis-shipments-tabs__header" role="tablist" aria-label="出库列表切换">
                ...
                <button type="button" class="aegis-shipments-tab" role="tab" aria-selected="false" data-tab="shipments">出库单列表（最近 7 天）</button>
            </div>
            <div class="aegis-shipments-tabs__panel is-active" role="tabpanel" data-panel="pending">
                <section class="aegis-card" style="margin:12px 0;">
                    ...
                    <?php if (empty($pending_orders)) : ?>
                        <p class="aegis-t-a6">当前无待出库订单。</p>
                        <?php if (!$order_link_enabled) : ?>
                            <p class="aegis-t-a6 aegis-shipments-tabs__note">订单关联未启用时无法显示订单数据。</p>
                        <?php endif; ?>
                    <?php else : ?>
            ...
            <div class="aegis-shipments-tabs__panel" role="tabpanel" data-panel="shipments" hidden>
...
<script>
    (function() {
        function initShipmentsTabs() {
            var container = document.querySelector('.aegis-shipments-page [data-aegis-tabs]');
            if (!container) {
                return;
            }
            ...
            tabs.forEach(function(tab) {
                tab.addEventListener('click', function() {
                    toggle(tab.getAttribute('data-tab'));
                });
            });
        }

        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initShipmentsTabs);
        } else {
            initShipmentsTabs();
        }
