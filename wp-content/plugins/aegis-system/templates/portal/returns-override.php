        ...
        <section class="aegis-card">
            ...
            <div class="aegis-card-body">
                <form method="post" class="aegis-t-a6" style="display:grid; gap:10px; max-width:680px;">
                    ...
                    <label>防伪码 code_value<input type="text" name="code_value" required style="width:100%;" /></label>
                    <label>经销商ID dealer_id<input type="number" name="dealer_id" min="1" required style="width:100%;" /></label>
                    <label>有效期（小时）<input type="number" name="expires_hours" min="1" max="168" value="48" style="width:100%;" /></label>
                    <label>通过原因<textarea name="reason_text" rows="3" required style="width:100%;"></textarea></label>
        ...
        <section class="aegis-card">
            ...
            <div class="aegis-card-body">
                <form method="get" class="aegis-t-a6" style="margin-bottom:12px; display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    ...
                    <label>防伪码<input type="text" name="search_code" value="<?php echo esc_attr($search_code); ?>" placeholder="精确匹配 code_value" /></label>
                ...
                <div class="aegis-table-wrap">
                    <table class="aegis-table aegis-t-a6" style="width:100%;">
                        <thead>
                            <tr><th>ID</th><th>防伪码</th><th>经销商</th><th>状态</th><th>expires_at</th><th>issued_at / issued_by</th><th>used_at / used_in_request_id</th><th>原因</th><th>操作</th></tr>
                        </thead>
                        <tbody>
                            <?php if (empty($rows)) : ?>
                            ...
                            <?php else : ?>
                                ...
                                <?php foreach ($rows as $row) : ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo esc_html(AEGIS_System::format_code_display((string) ($row->code_value ?? ''))); ?></td>
                                        <td><?php echo esc_html(($row->dealer_name ?? '-') . ' (#' . (int) $row->dealer_id . ')'); ?></td>
                                        <td><?php echo esc_html($ui_status); ?></td>
                                        <td style="white-space:nowrap;"><?php echo esc_html((string) ($row->expires_at ?? '')); ?></td>
                                        <td style="white-space:nowrap;"><?php echo esc_html((string) ($row->issued_at ?? '')); ?> / #<?php echo esc_html((string) ((int) $row->issued_by)); ?></td>
                                        <td style="white-space:nowrap;"><?php echo esc_html((string) ($row->used_at ?? '')); ?> / #<?php echo esc_html((string) ((int) $row->used_in_request_id)); ?></td>
                                        <td><?php echo esc_html(wp_trim_words((string) ($row->reason_text ?? ''), 20, '...')); ?></td>
                                        <td>
                                            <?php if ($can_revoke && $is_active_not_expired) : ?>
                                                <div class="table-actions">
                                                    <form method="post" class="inline-form" onsubmit="return confirm('确认撤销该特批码？');">
                                                        <?php wp_nonce_field('aegis_returns_override_action', 'aegis_returns_override_nonce'); ?>
