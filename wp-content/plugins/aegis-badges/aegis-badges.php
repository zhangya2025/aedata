...
if ( ! class_exists( 'Aegis_Badges' ) ) {
	...
	public function register_hooks() {
		...
		if ( $settings['mode'] === 'hide' ) {
			...
			add_filter( 'woocommerce_blocks_product_grid_item_html', array( $this, 'filter_blocks_grid_item' ), 10, 3 );
			return;
		}
	...
	public function filter_blocks_grid_item( $html, $data, $product ) {
		...
		$clean_html = $this->strip_blocks_sale_badge( $html );
		...
		return $this->inject_badge_into_block_item( $clean_html, $badge_html );
	}
	...
	private function get_badge_html( $product ) {
		...
		return '<span class="aegis-badge aegis-badge--preset-' . esc_attr( $preset ) . '" data-preset="' . esc_attr( $preset ) . '">' . esc_html( $label ) . '</span>';
	}

	private function strip_blocks_sale_badge( $html ) {
		$patterns = array(
	...
	private function inject_badge_into_block_item( $html, $badge_html ) {
		if ( preg_match( '/<\/li>\s*$/', $html ) ) {
			return preg_replace( '/<\/li>\s*$/', $badge_html . '</li>', $html, 1 );
		}
